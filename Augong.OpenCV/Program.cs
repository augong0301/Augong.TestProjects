...
class Program
{
	static void Main(string[] args)
	{
		...
		while (Console.ReadLine() != "end")
		{
			try
			{
				...
				// switch mode
				Console.WriteLine("Select file mode : zip ; png ; folder");
	...
	{
		Console.WriteLine("Merging Png Not supported yet, lol");
	}
	...
	static IEnumerable<T[]> GetCombinations<T>(T[] array, int combinationSize)
	{
		int[] indices = Enumerable.Range(0, combinationSize).ToArray();
		while (indices[0] < array.Length - combinationSize + 1)
		{
			yield return indices.Select(i => array[i]).ToArray();

			int i;
			for (i = combinationSize - 1; i >= 0 && indices[i] == array.Length - combinationSize + i; i--) ;
			if (i >= 0)
			{
				indices[i]++;
